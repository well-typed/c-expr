...
fromExpr ::
     forall ctx m var p. (Applicative m, Show var)
  => M.Expr var ctx p
  -> m (Expr var)
fromExpr = go
  where
    go :: M.Expr var ctx p -> m (Expr var)
    go = \case
      M.Term (M.Literal x) ->
        fromLit x
      M.Term (M.LocalParam i) ->
        panicPure $ show $ UnexpectedLocalParameterInType @var (idxToInt i)
      M.Term (M.Var _ nm []) ->
        pure $ Var nm
      M.Term (M.Var _ nm _ ) ->
      ...
      M.VaApp _ fun _ ->
        panicPure $ show $ UnexpectedValueFunctionApplicationInType @var (show fun)

    fromLit :: M.Literal -> m (Expr var)
    fromLit = \case
      M.TypeLit x  -> pure $ TypeLit x
      M.ValueLit x -> panicPure $ show $ UnexpectedValueLiteralInType @var (show x)
    ...
    myHead = \case
      (x ::: VNil) ->
        x
      (_ ::: _ ::: _) ->
        panicPure $ show $ UnexpectedMultipleArgumentsToUnaryTypeFunction @var
